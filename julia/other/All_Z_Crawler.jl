...
function diff_f(ChannelType, n, p, customP)
...
function diff_mean(ChannelType, n, p, customP; reps::Int=3)
    s = 0.0
    for _ in 1:reps
        s += diff_f(ChannelType, n, p, customP)
    end
    return s / reps
end
...
function bracket_root_local(ChannelType, n, p0, customP;
                            step::Float64=1e-3,
                            max_steps::Int=500,
                            reps::Int=3,
                            p_min::Float64=0.0,
    ...
    f0 = safe_diff_mean(ChannelType, n, p0, customP; reps=reps)
    if !isfinite(f0)
    ...
    for k in 1:max_steps
        ...
        pR_new = p0 + k*step
        if pR_new <= p_max
            fR_new = safe_diff_mean(ChannelType, n, pR_new, customP; reps=reps)
        ...
        pL_new = p0 - k*step
        if pL_new >= p_min
            fL_new = safe_diff_mean(ChannelType, n, pL_new, customP; reps=reps)
...
function bisect_threshold(ChannelType, n, p_lo, p_hi, customP;
                          p_tol::Float64=1e-6, reps::Int=7)

    # We will enforce invariant: f(lo) < 0, f(hi) > 0
    noise_lo, f_lo = estimate_noise(ChannelType, n, p_lo, customP; reps=reps)
    noise_hi, f_hi = estimate_noise(ChannelType, n, p_hi, customP; reps=reps)
    ...
    while (p_hi - p_lo) > p_tol
        ...
        noise_mid, f_mid = estimate_noise(ChannelType, n, p_mid, customP; reps=reps)
...
function envelope_finder(n_max, ChannelType, customP; pz=0.23,
    ...
    for n in 2:n_max
        ...
        elapsed_total = @elapsed begin
            p_lo, p_hi = bracket_root_local(ChannelType, n, pz, customP;
                                            step=scan_step, max_steps=800,
                                            reps=scan_reps,
                                            p_min=p_min, p_max=p_max)

            p_star = bisect_threshold(ChannelType, n, p_lo, p_hi, customP;
...
function main()
    ...
    n_max = 16
    hashing, base_grid = envelope_finder(n_max, ChannelType, ninexz)
end

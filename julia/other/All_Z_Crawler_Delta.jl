...
function all_z_code_checkn(pz, channelParamFunc) 
    S = falses(1,2n)
    S[1,(n+1):end] .= true 
    S = Matrix{Bool}(S)
    hb_induced = QECInduced.check_induced_channel(S, pz, channelParamFunc;  sweep=false, threads = 0)
    return hb_induced
end 
...
function diff_marginn(p, channelParamFunc, delta)
    hb = all_z_code_checkn(p, channelParamFunc)
    h  = hashing_calc(p, channelParamFunc)
    d  = (hb - h) - delta
    return isfinite(d) ? d : NaN
end

function diff_margin_meann(p, channelParamFunc, delta; reps::Int=3)
    s = 0.0
    cnt = 0
    for _ in 1:reps
        d = diff_marginn(p, channelParamFunc, delta)
        if isfinite(d)
            s += d
            cnt += 1
        end
...
function estimate_noise_marginn(p, channelParamFunc, delta; reps::Int=7)
    vals = Vector{Float64}(undef, reps)
    ok = 0
    for i in 1:reps
        v = diff_marginn(p, channelParamFunc, delta)
        vals[i] = v
        ok += isfinite(v) ? 1 : 0
    end
    if ok == 0
...
function estimate_noisen(p, channelParamFunc; reps::Int=7)
    vals = Vector{Float64}(undef, reps)
    for i in 1:reps
        vals[i] = diff_fn(p, channelParamFunc)
    end
    return (maximum(vals) - minimum(vals)), sum(vals) / reps
end

# bracket a sign change around p0 by scanning outward
function bracket_rootn, p0, channelParamFunc;
                      step::Float64=1e-3, max_steps::Int=2000, reps::Int=3)

    f0 = diff_meann(p0, channelParamFunc; reps=reps)

    # Scan upward from p0
    p_prev, f_prev = p0, f0
    for k in 1:max_steps
        p = p0 + k*step
        f = diff_meann(p, channelParamFunc; reps=reps)
...
function bracket_root_local_marginn, p0, channelParamFunc, delta;
                                   step::Float64=1e-3,
                                   max_steps::Int=800,
                                   reps::Int=3,
                                   p_min::Float64=0.0,
    ...
    f0 = diff_margin_meann(p0, channelParamFunc, delta; reps=reps)
    if !isfinite(f0)
    ...
    for k in 1:max_steps
        ...
        pR_new = p0 + k*step
        if pR_new <= p_max
            fR_new = diff_margin_meann(pR_new, channelParamFunc, delta; reps=reps)
        ...
        pL_new = p0 - k*step
        if pL_new >= p_min
            fL_new = diff_margin_meann(pL_new, channelParamFunc, delta; reps=reps)
...
function bisect_threshold_marginn, p_lo, p_hi, channelParamFunc, delta;
                                 p_tol::Float64=1e-6, reps::Int=7)

    noise_lo, f_lo = estimate_noise_marginn(p_lo, channelParamFunc, delta; reps=reps)
    noise_hi, f_hi = estimate_noise_marginn(p_hi, channelParamFunc, delta; reps=reps)
    ...
    while (p_hi - p_lo) > p_tol
        p_mid = (p_lo + p_hi)/2
        noise_mid, f_mid = estimate_noise_marginn(p_mid, channelParamFunc, delta; reps=reps)
...
function safe_diffn(p, channelParamFunc)
    hb = all_z_code_checkn(p, channelParamFunc)
    h  = hashing_calc(p, channelParamFunc)
    d  = hb - h
    if !isfinite(d)
...
function safe_diff_meann(p, channelParamFunc; reps::Int=3)
    s = 0.0
    cnt = 0
    for _ in 1:reps
        d = safe_diffn(p, channelParamFunc)
        if isfinite(d)
            s += d
            cnt += 1
        end
...
function root_finder_margin(n_max, channelParamFunc;
                                pz=0.23,
                                delta=DEFAULT_DELTA,
    ...
    for n in 2:n_max
        ...
        elapsed_total = @elapsed begin
            p_lo, p_hi = bracket_root_local_marginn, pz, channelParamFunc, delta;
                                                   step=scan_step, max_steps=800,
                                                   reps=scan_reps, p_min=p_min, p_max=p_max)

            p_star = bisect_threshold_marginn, p_lo, p_hi, channelParamFunc, delta;
...
function main()
    n_max = 16
    hashing, base_grid = root_finder_margin(n_max, Channels.Independent_Skewed_X_Nine)
end
